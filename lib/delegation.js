...
function getOwnNode() {
  ...
  console.log('Getting own node...');

  if (config.delegate.ownNodes) {
    console.log('Using own nodes env variable');
    nodes = JSON.parse(config.delegate.ownNodes);
  } else {
    console.log('Using config file');
    let dailyConfig;
    try {
      dailyConfig = JSON.parse(fs.readFileSync(config.nodeConfigPath, 'utf8'));
    } catch (e) {
      throw new Error(`Could not open node config file: ${config.nodeConfigPath}`);
    }

    const genesisState = dailyConfig?.final_chain?.state?.dpos?.genesis_state || {};
