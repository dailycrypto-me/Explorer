  ...
  return (
    <>
      ...
      <Card style={{ margin: 5, marginTop: 0, marginBottom: 10 }} bg="dark" text="white">
        <Card.Body>
          <Card.Title>Balance: {(address.balance / 1e18).toFixed(6)} DLY</Card.Title>
          <ul>
            <li>Received: {(address.received / 1e18).toFixed(6)} DLY</li>
            <li>Sent: {(address.sent / 1e18).toFixed(6)} DLY</li>
            <li>Fees: {(address.fees / 1e18).toFixed(6)} DLY</li>
          </ul>
        </Card.Body>
        <Card.Body>
              ...
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx._id}>
                    ...
                    <td>{(tx.value / 1e18).toFixed(6)} DLY</td>
                    <td>{((tx.gasUsed * tx.gasPrice) / 1e18).toFixed(6)} DLY</td>
                  </tr>
                ))}
              </tbody>
            </Table>
