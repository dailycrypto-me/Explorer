...
async function realtimeSync() {
  ...
  let subscribed = {};
  const ws = new WebSocket(config.daily.node.ws);
...
async function historicalSync(subscribed = false) {
  ...
  while (syncState.number < chainState.number) {
    ...
    if (block.number === '0x0') {
      let genesis = dailyConfig?.chain_config?.final_chain?.state?.genesis_balances || {};
...
const blockchainSync = async (cmdObj) => {
  ...
  try {
    dailyConfig = require(cmdObj.config);
  } catch (e) {
...
program.option('-c, --config <string>', 'path to daily config file');
